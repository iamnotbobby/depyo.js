...
function handleStoreDerefA() {
    if (this.unpack) {
        ...
        if (nameNode.name === '__classdict__' && valueNode instanceof AST.ASTLocals) {
            return;
        }
...
function handleLoadFastLoadFastA() {
    ...
    const idx1 = isPackedNibbles ? ((packed >> 4) & 0x0F) : ((packed >> 8) & 0xFF);
    const idx2 = isPackedNibbles ? ((packed) & 0x0F) : (packed & 0xFF);
