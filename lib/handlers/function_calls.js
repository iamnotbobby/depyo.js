...
function handleInstrumentedCallA() {
    ...
    if (loadBuildClassFound) {
        let loadBuildClassIndex = -1;
        for (let idx = this.dataStack.length - 1; idx >= 0; idx--) {
            if (this.dataStack[idx] instanceof AST.ASTLoadBuildClass) {
                loadBuildClassIndex = idx;
                break;
            }
        }

        let hasNull = (loadBuildClassIndex + 1 < this.dataStack.length) && (this.dataStack[loadBuildClassIndex + 1] === null);
        let funcIndex = loadBuildClassIndex + (hasNull ? 2 : 1);
        let nameIndex = funcIndex + 1;
        let basesStartIndex = nameIndex + 1;

        if (this.dataStack.length > nameIndex) {
             let functionNode = this.dataStack[funcIndex];
             let name = this.dataStack[nameIndex];
             let bases = this.dataStack.slice(basesStartIndex);
             ...
             this.dataStack.splice(loadBuildClassIndex); 
...
function handleCallKwA() {
    // Python 3.13
    let argc = this.code.Current.Argument;
    let kwnames = this.dataStack.pop();
    
    let kwNamesList = [];
    if (kwnames instanceof AST.ASTTuple) {
        kwNamesList = kwnames.values.map(v => v.object?.toString() || v.toString());
    } else if (kwnames instanceof AST.ASTObject && kwnames.object.ClassName === 'Py_Tuple') {
         kwNamesList = kwnames.object.Value.map(v => v.toString());
    }

    let kwArgs = [];
    let posArgs = [];
    
    let numKw = kwNamesList.length;
    let numPos = argc - numKw;
    
    for (let i = numKw - 1; i >= 0; i--) {
        let val = this.dataStack.pop();
        kwArgs.unshift({key: new AST.ASTName(kwNamesList[i]), value: val});
    }
    
    for (let i = 0; i < numPos; i++) {
        posArgs.unshift(this.dataStack.pop());
    }
    ...
    if (global.g_cliArgs?.debug) {
        console.log(`[handleCallKwA] func type: ${func?.constructor?.name}`);
    }

    if (func instanceof AST.ASTLoadBuildClass) {
        // Python 3.11
        let classBodyFunc = posArgs[0];
        let className = posArgs[1];
        let bases = posArgs.slice(2);
        
        let callNode = new AST.ASTCall(classBodyFunc, [], []);

        let classNode = new AST.ASTClass(callNode, new AST.ASTTuple(bases), className);
        classNode.line = this.code.Current.LineNo;
    ...
    let node = new AST.ASTCall(func, posArgs, kwArgs);
